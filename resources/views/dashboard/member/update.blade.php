...
<div class="col-lg-8">
    <form action="{{ route('member.update', ['member'=>$data->nisn]) }}" method="post" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="mb-3">
            <label for="nisn" class="form-label">NISN</label>
            <input type="text" name="nisn" class="form-control @error('nisn') is-invalid @enderror" id="nisn" value="{{ old('nisn', $data->nisn) }}" required>
            @error('nisn')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="kelas" class="form-control @error('kelas') is-invalid @enderror" id="kelas" value="{{ old('kelas', $data->name) }}" required>
            @error('kelas')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3 d-flex flex-column">
            ...
            <input class="form-control @error('foto') is-invalid @enderror" name="foto" type="file" id="formFileMultiple" value="{{ old('foto',$data->foto) }}" multiple>
            <img src="{{ asset('storage/'.$data->foto) }}" alt="{{ $data->foto }}" srcset="" class="img-fluid w-50 img-thumbnail mt-3 align-self-center">
            @error('foto')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
